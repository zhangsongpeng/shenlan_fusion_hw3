...
class Sim(object):
    ...
    def __gen_data_from_pathgen(self):
        ...
        for i in range(self.sim_count):
            (accel, ba) = pathgen.acc_gen(self.fs[0], self.dmgr.ref_accel.data,
                                    self.imu.accel_err, vib_def)
            self.dmgr.add_data(self.dmgr.accel.name, accel, key=i)
            self.dmgr.add_data(self.dmgr.ba.name, ba, key=i)
            (gyro, bg) = pathgen.gyro_gen(self.fs[0], self.dmgr.ref_gyro.data,\
                                    self.imu.gyro_err)
            self.dmgr.add_data(self.dmgr.gyro.name, gyro, key=i)
            self.dmgr.add_data(self.dmgr.bg.name, bg, key=i)
            if self.imu.gps:
