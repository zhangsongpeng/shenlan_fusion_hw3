...
def acc_gen(fs, ref_a, acc_err, vib_def=None):
    ...
    # ks = np.array([[0.95, 0.0, 0.0], [0.03, 1.03, 0.0], [-0.04, 0.05, 1.05]])
    # ks = (np.mat(ks)).I
    # acc_bias = np.array([0.02, 0.01, -0.02])
    # ks = np.transpose(ks)
    # ref_a2 = np.dot(ref_a, ks)
    a_mea = ref_a + acc_bias + acc_bias_drift + acc_noise + acc_vib
    return (a_mea, acc_bias_drift)
...
def gyro_gen(fs, ref_w, gyro_err):
    ...
    # true + constant_bias + bias_drift + noise
    # ks = np.array([[1.03, 0.03, -0.02], [-0.02, 0.97, 0.03], [0.05, -0.03, 1.01]])
    # ks = (np.mat(ks)).I
    # gyro_bias = np.array([-0.01, 0.03, 0.02])
    # ks = np.transpose(ks)
    # ref_w2 = np.dot(ref_w, ks)
    w_mea = ref_w + gyro_bias + gyro_bias_drift + gyro_noise
    return (w_mea, gyro_bias_drift)
...
def bias_drift(corr_time, drift, n, fs):
    ...
    for i in range(0, 3):
        ...
        # if not math.isinf(bias_rrw[i]):
        # rrw_noise = np.random.randn(n, 3)
        # for j in range(1, n):
        #     sensor_bias_drift[j, i] = sensor_bias_drift[j-1, i] + 1e-4 * math.sqrt(0.01)* rrw_noise[j-1,i]
